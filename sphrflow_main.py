...
soln = sp.linalg.spsolve(PDE_mat,rhs_tor_odd)

tor_arr,dr_tor_arr,pol_arr,dr_pol_arr,dr2_pol_arr = LN_case.process_soln('tor',soln)

print('time ellapsed to build/solve PDE matrix: {:.2f}s'.format(time()-t0))
...
surf_power = 4*np.pi * ek* LN_case.r_end**2* np.trapz(np.sin(theta_grid) * 0.5*np.real(q_phi[0,:]*np.conjugate(tau_phi_r[0,:])),x=theta_grid,axis=0)
print(surf_power)
